...
## 6. Open gaps found while measuring

- **`symlink` is not implemented in the mount.** `cp -r` of any real tree containing symlinks fails with `Function not implemented`; the eval's `diff -r` correctly reports `differs` for exactly those entries (3 of 597 on `/usr/share/doc/git`). All 597 regular files round-tripped byte-identically. Needs `symlink`/`readlink` ops and a link-target field in metadata.
- **The reconciliation probe budget is unsound.** `bench_reconciliation` polls with no per-probe bound inside a 30 s deadline, so a single slow `stat` can consume the whole budget and be recorded as a propagation failure. It also never checks that the write it is timing succeeded. One eval run reported 5/5 sonic→office failures that did not reproduce (3/3 clean at 270 ms visible / 557 ms readable on re-run).
